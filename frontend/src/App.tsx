      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          <div className="flex flex-col gap-2">
            ...
            <textarea
              className="flex-1 min-h-[160px] p-3 rounded-xl bg-slate-900 border border-slate-700 text-slate-200 text-sm resize-none focus:outline-none focus:border-orange-500/60 placeholder:text-slate-600"
          ...
          <div className="md:col-span-2 flex items-center gap-4">
            <button
              ...
              className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-400 disabled:opacity-40 disabled:cursor-not-allowed text-slate-950 font-semibold text-sm transition-colors"
            >
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {[
              { step: '01', title: 'Claim Extraction', description: 'AI response is atomized into individual factual claims using spaCy sentencizer.', icon: FileText, color: 'text-amber-400', bgColor: 'bg-amber-500/10' },
              { step: '02', title: 'NLI Cross-Examination', description: 'Each claim is evaluated against every source sentence using DeBERTa NLI model.', icon: Zap, color: 'text-orange-400', bgColor: 'bg-orange-500/10' },
              { step: '03', title: 'Truth Classification', description: 'Claims are categorized as Faithful, Extrapolated, Fabricated, or Contradicted.', icon: AlertTriangle, color: 'text-red-400', bgColor: 'bg-red-500/10' },
              { step: '04', title: 'Risk Scoring', description: 'A composite risk index is calculated based on unsupported claim density.', icon: Zap, color: 'text-rose-400', bgColor: 'bg-rose-500/10' },
            ].map((item, idx) => (
              <motion.div
                key={item.step}
                initial={{ opacity: 0, y: 20 }}
